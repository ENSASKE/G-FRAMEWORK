...
class HTML {
    ...
    function link($text,$path,$prompt = null,$confirmMessage = "¿Estás seguro?") {
        $path = str_replace(' ','-',$path);
        if ($prompt) {
            $data = '<a href="javascript:void(0);" onclick="javascript:irA(\''.BASE_PATH.'/'.$path.'\',\''.$confirmMessage.'\')">'.$text.'</a>';
        } else {
    ...
    function includeCss($fileName) {
        $data = '<link href="'.BASE_PATH.'/public/css/'.$fileName.'.css" media="all" rel="stylesheet" type="text/css">';
        echo $data."\n";
        return $data;
    }
}